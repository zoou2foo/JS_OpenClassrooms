...
function validerNom(nom) {
  if (nom.length < 2) throw new Error(`Le nom: ${nom}, est trop petit`);
}
...
function launchGame() {
  ...
  boutonValidation.addEventListener("click", () => {
    ...
    if (inputEcriture.value === listeProp[i]) score++;
    i++;
  ...
  for (let index = 0; index < listBtnRadio.length; index++) {
    listBtnRadio[index].addEventListener("change", (event) => {
      //dans event store l'information si c'est le bouton de droite ou gauche qui a été cliqué
      //Si c'est le premier élément qui a été modifié, alors nous voulons
      //jouer avec la listeMots
      if (event.target.value === "1") {
        //target est la propriété où l'info est stocké, puis il faut juste appeler value
        listeProp = listeMots;
      } else {
